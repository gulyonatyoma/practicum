...
def m_constant_angle(obj, is_catch):
    table = obj.data
    if is_catch == False:
    ...
    flag = 0
    (x_aim_1, y_aim_1) = obj.math_f.straight_serif(
            obj.xb, obj.yb, obj.r0, obj.f0)
    q=obj.math_f.angle_between_lines(x_aim, y_aim, x_c, y_c, obj.xb, obj.yb, x_aim_1,y_aim_1)
    constnt_angle= degrees(asin(obj.v_aim *sin(radians(q))/obj.v_c))
    while d > obj.max_dist:
        ...
        (r_aim, f_aim) = obj.math_f.reverse_serif(
            obj.xb, obj.yb, x_aim, y_aim)
        if obj.v_aim<0:
            f_0=obj.math_f.angle_between_lines(x_c, y_c, x_aim, y_aim, x_c, y_c, x_c, 0)+constnt_angle
        else:
            f_0=obj.math_f.angle_between_lines(x_c, y_c, x_aim, y_aim, x_c, y_c, x_c, 0)-constnt_angle
        x_intersection, y_intersection=obj.math_f.straight_serif(x_c, y_c, abs(obj.v_c)+10, f_0)
        a= obj.math_f.circle_and_line(
            x_c , y_c, x_intersection, y_intersection, x_c, y_c, abs(obj.v_c))
