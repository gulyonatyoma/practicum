...
def m_straightening(obj, is_catch, m):
    table = obj.data
    if is_catch == False:
    ...
    while d > obj.max_dist:
        ...
        delta_s = abs(r_aim - r_c)
        H = abs(y_aim - y_c)
        S = abs(x_aim - x_c)
        if obj.v_aim < 0:
            d_delta_s = -abs(obj.v_aim) * cos(radians(f_aim)) - obj.v_c
            d_f_aim = -abs(obj.v_aim) / (H * (1 + (S/H)**2))
        else:
            d_delta_s = obj.v_aim * cos(radians(f_aim)) - obj.v_c
            d_f_aim = obj.v_aim / (H * (1 + (S/H)**2))
        if d_delta_s < 0:
            f_c = f_aim - m * d_f_aim * delta_s / abs(d_delta_s)
        else:
            f_c = f_aim + m * d_f_aim * delta_s / d_delta_s
        a = obj.math_f.circle_and_line(
            obj.xb, obj.yb, obj.xb + r_aim * cos(radians(f_c)), r_aim * sin(radians(f_c)), x_c, y_c, obj.v_c)
        if len(a) == 4:
